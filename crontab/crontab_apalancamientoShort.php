...
echo "\n".date('d/m/Y H:i:s')." Martingala Short";

if (!Operacion::lockProcess('Crontab::apalancamientoCruzadoShort()'))
{
    $lockFileText = Operacion::readLockFile();
    $msg = 'Error - Bot Apalancamiento Cruzado Short Bloqueado - '.$lockFileText;
...
foreach ($usuarios as $idusuario => $usuarioData)
{
    try {
        ...
        $msg = "Apalancamiento Short - Error: " . $e->getMessage();
        Operacion::logBot('u:'.$idusuario.' '.$msg);
        continue;
    }
}
