                ...
                $.each(data.tickers, function(i, ticker) {
                    tbody.append(`
                        <tr id="ticker_${ticker.tickerid}">
                           ...
                           <td class="text-right ${(ticker.perc_12h>0?'text-success':'text-danger')}">
                                ${(ticker.perc_12h?ticker.perc_12h:'')}</td>
                           <td class="text-right ${(ticker.perc_24h>0?'text-success':'text-danger')}">
                                ${(ticker.perc_24h?ticker.perc_24h:'')}</td>
                        </tr>`);
                });
            }
            else
